...
2. PO Guardian **decides per-request whether a Formal Spec is warranted** — multi-component / cross-Guardian / architectural shifts produce a Spec Kit-compatible spec at `specs/{feature}/spec.md`; trivial work skips it. Decision and rationale are captured in every ticket via the `Parent Spec:` field.
3. **For brownfield projects** — when the area being changed has no parent spec, the PO Guardian bootstraps one from the existing code so the new change has a baseline to evolve from. Bug fixes against an area with a spec **patch the spec** as part of the ticket.
4. **For non-trivial work** — the PO Guardian consults the Code Review Guardian for architectural-impact assessment, which feeds the spec's System Impact section.
5. PO Guardian presents the decomposition (and spec, if produced) to the user for approval
6. PO Guardian details each ticket with the standard ticket template (each ticket carries a `Parent Spec:` line)
