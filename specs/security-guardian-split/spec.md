...
- **A/B testing happens on the user's own machine** with manual orchestrator switching (env var or feature flag in the coordinator's instructions file). No automated A/B harness is built — the user runs both versions on equivalent inputs and reviews the diff manually. Acceptable because the volume of test runs (≥10) is small.
...
- **The monolithic Security Guardian and the new coordinator coexist on different branches during Phase 1 and Phase 2.** Switching is a `git checkout` away. No runtime feature flag is built — branch is the flag. (Acceptable because A/B testing is done by the project owner manually.)
